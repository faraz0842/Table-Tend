                ...
                <div id="kt_app_toolbar_container" class="app-container d-flex flex-stack">
                    <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                        ...
                        <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                            ...
                            <li class="breadcrumb-item text-muted">{{ __('razorpay') }}</li>
                        </ul>
                    </div>
                </div>
            </div>
                            ...
                            <div class="card card-flush pb-0 bgi-position-y-center bgi-no-repeat mb-10">
                                ...
                                <div class="card-body pb-0">
                                    ...
                                    <ul class="nav nav-tabs">
                                        ...
                                        <li class="nav-item ">
                                            <a class="nav-link" href="{{ Route('payments.stripe') }}"
                                                role="button" aria-expanded="false">{{ __('stripe') }}
                                                <span
                                                    class="badge badge-exclusive badge-light-success fw-semibold fs-8 px-2 py-1 ms-1"
                                                    data-bs-toggle="tooltip" data-bs-placement="right"
                                                    data-bs-original-title="In-house component"
                                    ...
                                    <form class="form" action="{{Route('payments.store.razorpay')}}" method="POST">
                                        @csrf
                                            ...
                                            <div class="row">
                                                <div class="col-lg-12 fv-row fv-plugins-icon-container my-10">
                                                    <label class="fw-bold fs-6">
                                                        {{ __('enable_razorpay') }}</label>
                                                    <input class="form-check-input mx-5" type="checkbox"
                                                        name="enable_razorpay" {{ isset($razorpay['app_razorpay']) && $razorpay['app_razorpay'] == 'Active' ? 'checked' : '' }}/>
                                                    <small>{{ __('check_it_to_enable_razorpay_payment_method') }}</small>
                                                    @if ($errors->has('enable_razorpay'))
                                                        <div class="text-danger">
                                                            {{ $errors->first('enable_razorpay') }}
                                                        </div>
                                                    @endif
                                                </div>
                                                <div class="col-lg-6 fv-row fv-plugins-icon-container">
                                                    <label class="col-lg-6 col-form-label fw-bold fs-6">
                                                        {{ __('razorpay_key') }}</label>
                                                    <input type="text" name="razorpay_key" class="form-control"
                                                    value="{{ isset($razorpay['app_razorpay_key']) ? $razorpay['app_razorpay_key'] : '' }}"/>
                                                    <small>{{ __('insert_razorpay_key') }}</small>
                                                ...
                                                <div class="col-lg-6 fv-row fv-plugins-icon-container">
                                                    <label class="col-lg-6 col-form-label fw-bold fs-6">
                                                        {{ __('razorpay_secret') }}</label>
                                                    <input type="text" name="razorpay_secret" class="form-control"
                                                            value="{{ isset($razorpay['app_razorpay_secret']) ? $razorpay['app_razorpay_secret'] : '' }}"/>
                                                    <small>{{ __('insert_razorpay_secret') }}</small>
