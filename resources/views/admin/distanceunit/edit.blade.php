                ...
                <div id="kt_app_toolbar_container" class="app-container d-flex flex-stack">
                    <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                        ...
                        <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                            ...
                            <li class="breadcrumb-item text-muted">{{ __('edit_distance_unit') }}</li>
                        </ul>
                    </div>
                </div>
            </div>
            <div id="kt_app_content" class="app-content flex-column-fluid">
                <div id="kt_app_content_container" class="app-container">
                    ...
                    <form class="form d-flex flex-column" method="POST" enctype="multipart/form-data"
                        action="{{ route('distanceUnit.update', $distanceUnit->slug) }}">
                        @csrf
                                ...
                                <div class="card card-flush py-4">
                                    ...
                                    <div class="card-body pt-0">
                                        <div class="col-lg-12 fv-row fv-plugins-icon-container">
                                            <label
                                                class="col-lg-12 col-form-label required fw-bold fs-6">{{ __('distance_unit_slug') }}</label>
                                            <input type="text" name="slug" class="form-control"
                                                placeholder="{{ __('enter_slug') }}"
                                                value="{{ old('slug', $distanceUnit->slug) }}">
                                            @if ($errors->has('slug'))
                                        ...
                                        @foreach (config('translatable.locales') as $translation)
                                            <div class="col-lg-12 fv-row fv-plugins-icon-container">
                                                <label
                                                    class="col-lg-12 col-form-label required fw-bold fs-6">{{ __('distance_unit_name') }}
                                                    ({{ $translation }})
                                                </label>
                                                <input type="text" name="{{ $translation }}[name]" class="form-control"
                                                    placeholder="{{ __('enter_category_name') }}"
                                                    value="{{ old($translation . '.name', $distanceUnit->hasTranslation($translation) ? $distanceUnit->translate($translation)->name : '') }}">
