      ...
      <div class="card-header border-0 pt-6">
          <div class="card-title">
              <div class="d-flex align-items-center position-relative my-1">
                  ...
                  <input type="text" wire:model="searchStatus" class="form-control form-control-solid w-250px ps-15"
                      placeholder="{{ __('search_status') }}" />
              </div>
          </div>
          @can('status.create')
              ...
              <tbody class="fw-semibold text-gray-600">
                  @forelse ($statuses as $key => $status)
                      <tr class="text-center">
                          ...
                          <td class="text-dark">{{ $status->slug }}</td>
                          <td class="text-dark">{{ $status->status }}</td>
                          <td class="text-dark">
                            @can('status.edit')
                            ...
                            @endcan
                            @can('status.delete')
                              <a type="button" data-bs-toggle="modal" data-category="{{ $status['slug'] }}"
                                  data-bs-target="#deleteModal{{ $status['slug'] }}"
                      ...
                      <div class="modal fade" id="deleteModal{{ $status['slug'] }}">
                              ...
                              <div class="modal-content">
                                  ...
                                  <div class="modal-footer d-flex justify-content-center">
                                      <button type="button" class="btn btn-light"
                                          ...
                                          {{ __('cancel') }}</button>
                                      <a href="{{ route('status.delete', $status['slug']) }}" type="button"
                  ...
                  @empty
                      <tr>
                          <td colspan="4" class="text-center">
                              <h4>{{ __('no_order_status_found') }}</h4>
                          </td>
                      </tr>
                  @endforelse
              </tbody>
