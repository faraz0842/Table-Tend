        ...
        @can('faqCategory.create')
            <div class="card-toolbar">
                <div class="d-flex justify-content-end">
                    <a href="{{ route('faqCategory.create') }}" class="btn btn-primary">
            ...
            <tbody class="fw-semibold text-gray-600">
                @forelse ($faqCategories as $key => $faqCategory)
                    <tr class="text-center">
                        ...
                        <td class="text-dark">{{ $faqCategory->slug }}</td>
                        <td class="text-dark">{{ $faqCategory->name }}</td>
                        <td class="text-dark">
                            @can('faqCategory.edit')
                                <a href="{{ route('faqCategory.edit', $faqCategory->slug) }}"
                                    class="btn btn-icon btn-success btn-sm mr-2">
                                    <i class="bi bi-pencil fs-4"></i>
                                </a>
                            @endcan
                            @can('faqCategory.delete')
                                <a type="button" data-bs-toggle="modal" data-category="{{ $faqCategory['slug'] }}"
                                    data-bs-target="#deleteModal{{ $faqCategory['slug'] }}"
                    ...
                    <div class="modal fade" id="deleteModal{{ $faqCategory['slug'] }}">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-footer d-flex justify-content-center">
                                    <button type="button" class="btn btn-light"
                                        ...
                                        {{ __('cancel') }}</button>
                                    <a href="{{ route('faqCategory.delete', $faqCategory['slug']) }}" type="button"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="text-center">
                            <h4>{{ __('no_faq_category_found') }}</h4>
                        </td>
                    </tr>
                @endforelse
            </tbody>
